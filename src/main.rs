...
use clap::Parser;
use cli::{Cli, CliFlags, Commands, EnvAction, KeyType, SecretAction, SetTargetArgs, SourceAction};
use crypto::{
    check_openssl_available, generate_hs256_key, generate_password, generate_rsa_keypair,
};
                ...
                SecretAction::Generate {
                    ...
                    match key_type {
                        KeyType::Pem => {
                            ...
                            if let Err(e) = check_openssl_available() {
                            ...
                            let private_key_name = format!("{}_PRIVATE", key);
                            let public_key_name = format!("{}_PUBLIC", key);

                            // Check for existing keys
                            let existing_private = vault.get_in_env(&env, &private_key_name);
                            let existing_public = vault.get_in_env(&env, &public_key_name);
                            ...
                            if flags.dry_run {
                                print_out(
                                    flags,
                                    &format!(
                                        "(dry-run) Would generate RSA 2048-bit PEM key pair '{}' in env '{}'",
                                        key, env
                                    ),
                                );
                                return Ok(());
                            ...
                            let keypair = generate_rsa_keypair().map_err(|e| {
                                AppError::new(ExitCode::VaultError, anyhow::anyhow!("{}", e))
                            })?;

                            let description = "RSA 2048-bit key pair (generated by cred)";
                            ...
                            if flags.json {
                                ...
                                print_out(
                                    flags,
                                    &format!(
                                        "✓ Generated RSA 2048-bit PEM key pair '{}' in env '{}' (stored as '{}' and '{}')",
                                        key, env, private_key_name, public_key_name
                                    ),
                                );
                            }
                        }
                        KeyType::Password => {
                            ...
                            let existing = vault.get_in_env(&env, &key);

                            if existing.is_some() && !force {
                                return Err(AppError::user(anyhow::anyhow!(
                                    "Secret '{}' already exists in env '{}'. Use --force to overwrite.",
                                    key,
                                    env
                                )));
                            }
                            ...
                            if flags.dry_run {
                                print_out(
                                    flags,
                                    &format!(
                                        "(dry-run) Would generate {}-character password '{}' in env '{}'",
                                        length, key, env
                                    ),
                                );
                                return Ok(());
                            ...
                            let password = generate_password(length).map_err(|e| {
                                AppError::new(ExitCode::VaultError, anyhow::anyhow!("{}", e))
                            })?;

                            let description =
                                format!("Random {}-character password (generated by cred)", length);
                            ...
                            if flags.json {
                                ...
                                print_out(
                                    flags,
                                    &format!(
                                        "✓ Generated {}-character password '{}' in env '{}'",
                                        length, key, env
                                    ),
                                );
                            }
                        ...
                        KeyType::Hs256 => {
                            ...
                            if flags.dry_run {
                                print_out(
                                    flags,
                                    &format!(
                                        "(dry-run) Would generate HMAC-SHA256 key '{}' in env '{}'",
                                        key, env
                                    ),
                                );
                                return Ok(());
                            ...
                            let hs256_key = generate_hs256_key().map_err(|e| {
                                AppError::new(ExitCode::VaultError, anyhow::anyhow!("{}", e))
                            })?;

                            let description = "HMAC-SHA256 key (generated by cred)";

                            // Store the key with base64 format
                            vault.set_with_metadata_in_env(
                                &env,
                                &key,
                                &hs256_key,
                                vault::SecretFormat::Base64,
                                Some(description.to_string()),
                                Some("generated".to_string()),
                                None,
                                None,
                            ...
                            if flags.json {
                                ...
                                print_out(
                                    flags,
                                    &format!(
                                        "✓ Generated HMAC-SHA256 key '{}' in env '{}' (base64 encoded)",
                                        key, env
                                    ),
                                );
                            }
